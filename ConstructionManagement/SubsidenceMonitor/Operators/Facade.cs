...
namespace PmSoft.ConstructionManagement.SubsidenceMonitor.Operators
{
    public class Facade
    {
        ...
        public static BLLResult CreateDetail(TList list, TDetail detail, List<TNode> nodes)
        {
            ...
            using (var connection = SQLiteHelper.Connect())
            {
                ...
                try
                {
                    ...
                    if (!nodes.Select(c => new TNodeElement(c.Segregation, c.NodeCode) { ElementIds = c.GetElementIds() }).DbInsertOrReplace(connection))
        ...
        public static BLLResult CreateDetail(TList list, TDetail detail, List<TDepthNode> depthNodes)
        {
            var result = new BLLResult();
            using (var connection = SQLiteHelper.Connect())
            {
                ...
                try
                {
                    ...
                    if (!detail.DbInsert(connection))
                        throw new NotImplementedException("Detail数据未按预期存储");
                    //nodes.DbDeleteByDetailKey(connection);//由于提交时,Nodes数量发生了变更,这里省略对原有Nodes数量删除的监测
                    if (!depthNodes.DbInsert(connection))
                        throw new NotImplementedException("Nodes数据未按预期存储");
                    //0711TODO KO 更新TNodeElement
                    if (!depthNodes.Select(c => new TDepthNodeElement(c.Segregation, c.NodeCode, c.Depth) { ElementIds = c.GetElementIds() }).DbInsertOrReplace(connection))
                        throw new NotImplementedException("NodeElements数据未按预期存储");
                    transaction.Commit();
                }
                catch (Exception ex)
        ...
        public static void FetchNodeElements(TDetail detail)
        {
            using (var connection = SQLiteHelper.Connect())
            {
                connection.Open();
                detail.FetchNodeElements(connection);
                connection.Close();
            }
        }
        ...
        public static void FetchDepthNodeElements(TDetail detail)
        {
            using (var connection = SQLiteHelper.Connect())
            {
                connection.Open();
                detail.FetchDepthNodeElements(connection);
                connection.Close();
            }
        }
