...
namespace PmSoft.ConstructionManagement.SubsidenceMonitor.Operators
{
    public static partial class EntityOperator
    {
        ...
        public static void FetchDetails(this TList entity, SQLiteConnection connection)
        {
            var command = connection.CreateCommand();
            var detail = new TDetail();
            List<KeyOperatorValue> wheres = new List<KeyOperatorValue>();
            wheres.Add(new KeyOperatorValue(nameof(entity.Segregation), SQLiteOperater.Eq, SQLiteHelper.ToSQLiteString(UniqueIdHelper.UniqueIdStr)));
            wheres.Add(new KeyOperatorValue($"datetime({nameof(detail.IssueDateTime)},'start of day')", SQLiteOperater.Eq, SQLiteHelper.ToSQLiteString(entity.IssueDate)));
            wheres.Add(new KeyOperatorValue(nameof(detail.IssueType), SQLiteOperater.Eq, SQLiteHelper.ToSQLiteString<EIssueType>(entity.IssueType)));
            command.CommandText = SQLiteHelper.GetSQLiteQuery_Select(null, detail.TableName, wheres);
            var reader = command.ExecuteReader();
            while (reader.Read())
            {
                entity.Datas.Add(new TDetail(reader) { List = entity });
            }
        }
        #endregion
    }
